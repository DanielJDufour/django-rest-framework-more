...
def create_model_filter_form(model, debug=False):
    ...
    if debug: print("filterable_fields:", filterable_fields)

    filterable_field_names = [f.name for f in filterable_fields if f.__class__.__name__ in ("BooleanField", "CharField", "TextField")]
    if debug: print("filterable_field_names:", filterable_field_names)

    form_name = model_name + "Form"
    if debug: print("form_name:", form_name)
...
def create_model_filterset_class(form=None, model=None, debug=False):
    ...
    if debug: print("filter_name:", filter_name)
    ...
    if debug: print("filter_defs:", filter_defs)

    filterset_class = type(filter_name, (FilterSet,), filter_defs)
    if debug: print("filterset_class:", filterset_class)
